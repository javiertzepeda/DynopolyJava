...
public class Main{
	...
	public static void main(String[] args) {
		...
		System.out.printf("Player %d has been randomly selected to go first.%n", currentPlayer+1);
		...
		while (!hasGameEnded()) {
			...
			System.out.printf("It is player %d's turn.%n", currentPlayer + 1);
			//MortgageCheck(); /* Uncomment to ask player if they would like to mortgage/unmortgage before they roll */
			System.out.printf("Player %d has rolled a %d and a %d.%n", currentPlayer + 1, die1, die2);
			if (!isPlayerJailed()) { // Before moving player, checks to see if a Player is in jail and needs to pay a fine
				...
				System.out.printf("Player %d moves %d spaces and lands on %s.%n", currentPlayer + 1, die1 + die2, currentPlayerLocation);
